...
inter_wl_toggle_fullscreen(struct InterDecoration *libdecor_client,
    ...
    if (!libdecor_client->is_fullscreen) {
        ...
        if (window_listeners && window_listeners->fullscreen) {
            window_listeners->fullscreen();
        }
        return;
    }
    libdecor_frame_unset_fullscreen(libdecor_client->frame);
    libdecor_client->is_fullscreen = false;
}
...
init_libdecor(struct InterWaylandClient *wlclient) {
    ...
    wlclient->libdecor->frame =
        libdecor_decorate(wlclient->libdecor->base, wlclient->wayland->surface,
                          &frame_events, wlclient);
