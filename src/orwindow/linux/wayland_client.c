...
static void register_device(void* data, struct wl_registry* registry, uint32_t name, const char* interface,
                            uint32_t version)
{
    struct InterWaylandClient* client = data;
    if (strcmp(interface, wl_compositor_interface.name) == 0)
    {
        client->wayland->compositor = wl_registry_bind(registry, name, &wl_compositor_interface, version);
    }
    else if (strcmp(interface, wl_shm_interface.name) == 0)
    {
        client->wayland->shared_memory = wl_registry_bind(registry, name, &wl_shm_interface, version);
    }
    else if (strcmp(interface, wl_seat_interface.name) == 0)
    {
        client->wayland->seat = wl_registry_bind(registry, name, &wl_seat_interface, SEAT_VERSION);
        if (client->listeners)
        {
            if (client->listeners->keyboard_listeners)
            {
                setup_keyboard(client->listeners->window_listeners, client->listeners->keyboard_listeners,
                               client->wayland);
            }
            if (client->listeners->pointer_listeners)
            {
                setup_pointer(client->listeners->pointer_listeners, client->wayland);
            }
        }
    }
}
...
static void unregister_device(void* data, struct wl_registry* registry, uint32_t name)
{
...
void inter_draw(const struct ORBitmap* bitmap, const struct ORWindowListeners* window_listeners)
{
...
void inter_frame_render(const struct InterWaylandClient* wlclient)
{
    ...
    wl_surface_damage_buffer(wlclient->wayland->surface, 0, 0, wlclient->bitmap->width, wlclient->bitmap->height);
...
enum ORWindowError inter_wl_window_resize(const struct InterWaylandClient* wlclient)
{
    const int32_t file_descriptor =
        shm_open(FILE_DESCRIPTOR_NAME, O_RDWR | O_CREAT | O_EXCL, S_IWUSR | S_IRUSR | S_IWOTH | S_IROTH);
    shm_unlink(FILE_DESCRIPTOR_NAME);
    ftruncate(file_descriptor, wlclient->bitmap->mem_size);
    wlclient->bitmap->mem = mmap(0, wlclient->bitmap->mem_size, PROT_READ | PROT_WRITE, MAP_SHARED, file_descriptor, 0);
    ...
    struct wl_shm_pool* pool =
        wl_shm_create_pool(wlclient->wayland->shared_memory, file_descriptor, wlclient->bitmap->mem_size);
    if (pool == NULL)
    {
    ...
    wlclient->wayland->buffer = wl_shm_pool_create_buffer(pool, 0, wlclient->bitmap->width, wlclient->bitmap->height,
...
enum ORWindowError inter_wl_window_setup(struct ORBitmap* bmp, struct InterWaylandClient* wlclient)
{
    if (wlclient == NULL)
    {
        return OR_DISPLAY_INIT_ERROR;
