                           ...
                           uint32_t state)
{
    ...
    if (state == WL_POINTER_BUTTON_STATE_PRESSED && pointer_listeners.button_press)
    {
                         ...
                         uint32_t time, uint32_t axis, wl_fixed_t value)
{
    ...
    else if (axis == WL_POINTER_AXIS_HORIZONTAL_SCROLL && pointer_listeners.scroll_y)
    {
...
void setup_pointer(const struct ORPointerListeners* pt_listeners, struct InterWayland* wayland)
{
